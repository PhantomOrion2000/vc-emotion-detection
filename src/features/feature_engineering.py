...
def apply_tfidf(X_train, X_test, y_train, y_test, max_features: int) -> tuple:
    try:
        vectorizer = TfidfVectorizer(max_features=max_features)
        X_train_tfidf = vectorizer.fit_transform(X_train)
        X_test_tfidf = vectorizer.transform(X_test)
        train_df = pd.DataFrame(X_train_tfidf if isinstance(X_train_tfidf, np.ndarray) else X_train_tfidf.toarray())
        train_df['label'] = y_train
        test_df = pd.DataFrame(X_test_tfidf if isinstance(X_test_tfidf, np.ndarray) else X_test_tfidf.toarray())
...
def main():
    try:
        ...
        train_df, test_df = apply_tfidf(X_train, X_test, y_train, y_test, params["max_features"])
